        ...
        <div className="bg-gradient-to-br from-[#1e293b]/70 to-[#1e293b]/40 border border-gray-700/50 backdrop-blur-sm rounded-lg shadow-xl p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">
              Welcome back,{" "}
              <span className="text-teal-400">{getUserName()}</span>!
            </h1>
            <div className="flex gap-2">
              <Link
                to="/"
                className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-teal-500/20 text-teal-400 hover:bg-teal-500/30 rounded-lg transition-colors text-sm sm:text-base"
              >
...
function DashboardCard({ title, description, to, icon }) {
  return (
    ...
    >
      ...
      <h3 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2 text-teal-400 group-hover:text-teal-300 transition-colors">
        {title}
      </h3>
